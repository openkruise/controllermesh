...
import (
	...
	"github.com/openkruise/controllermesh/webhook/util/generator"
	"github.com/openkruise/controllermesh/webhook/util/writer"
...
const (
	mutatingWebhookConfigurationName   = "ctrlmesh-mutating-webhook-configuration"
	validatingWebhookConfigurationName = "ctrlmesh-validating-webhook-configuration"
)
...
func (c *Controller) ensureConfigurations(caBundle []byte) error {
	mutatingConfig, err := c.kubeClient.AdmissionregistrationV1().MutatingWebhookConfigurations().Get(context.TODO(), mutatingWebhookConfigurationName, metav1.GetOptions{})
	if err != nil {
	...
	if !reflect.DeepEqual(mutatingConfig, oldMutatingConfig) {
		if _, err := c.kubeClient.AdmissionregistrationV1().MutatingWebhookConfigurations().Update(context.TODO(), mutatingConfig, metav1.UpdateOptions{}); err != nil {
